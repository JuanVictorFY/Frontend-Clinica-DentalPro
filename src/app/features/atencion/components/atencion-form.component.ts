        ...
        @if (cita()) {
          ...
          <form [formGroup]="notaForm" (ngSubmit)="onSubmit()" class="space-y-5 p-6 bg-gray-900 rounded-xl border border-gray-700">
            ...
            <div class="flex items-start gap-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
              <svg class="w-4 h-4 text-yellow-400 mt-0.5 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
              </svg>
              <p class="text-xs text-yellow-300">Al finalizar, se creará un pago <strong>pendiente</strong> automáticamente. La recepción podrá cobrarlo desde el módulo de Pagos.</p>
            </div>
...
export class AtencionFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.citaService.obtenerPorId(Number(id)).subscribe({
      next: (cita) => {
  ...
  onSubmit(): void {
    ...
    this.atencionService.registrarNota({
      ...
      next: () => {
        this.toast.success('Atención finalizada. Pago pendiente generado.');
